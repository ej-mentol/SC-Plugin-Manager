...
__fastcall TAddPluginForm::TAddPluginForm(TComponent* Owner)
    : TForm(Owner)
{
...
void __fastcall TAddPluginForm::SetFileList(const TStringDynArray& plugins, const String& basePath)
{
	FBasePath = basePath.Trim();
	while (!FBasePath.IsEmpty() && (FBasePath[FBasePath.Length()] == L'\\' || FBasePath[FBasePath.Length()] == L'/'))
...
void TAddPluginForm::UpdateFilteredList(const String& filterText)
{
    ...
    if (filterText.IsEmpty())
    {
		...
		for (int i = 0; i < std::min(FPluginsList->Count, MaxItemsWithoutFilter); i++)
		{
			String fullPath = FPluginsList->Strings[i];
			String relPath = GetRelativePath(fullPath);
...
void __fastcall TAddPluginForm::FilterListBoxItemClick(TCustomListBox * const Sender, TListBoxItem * const Item)
{
    ...
    if (index >= 0 && index < FFilteredPlugins->Count)
    {
		...
		if (NameEdit->Text.Trim().IsEmpty())
        {
			String suggestedName = System::Ioutils::TPath::GetFileNameWithoutExtension(fullPath);
...
void __fastcall TAddPluginForm::ScriptEditChangeTracking(TObject *Sender)
{
	...
	OkButton->Enabled = (!ScriptEdit->Text.IsEmpty() && !NameEdit->Text.IsEmpty());
}

void __fastcall TAddPluginForm::ScriptEditKeyDown(TObject *Sender, WORD &Key, System::WideChar &KeyChar, TShiftState Shift)
{
	if (!FilterPopup->IsOpen)
		return;
	...
	case vkTab:
		if (FilterListBox->ItemIndex >= 0)
		{
			TListBoxItem* selectedItem = dynamic_cast<TListBoxItem*>(FilterListBox->ListItems[FilterListBox->ItemIndex]);
			if (selectedItem)
...
void __fastcall TAddPluginForm::NameEditKeyDown(TObject *Sender, WORD &Key, System::WideChar &KeyChar,
          TShiftState Shift)
{
 if( Key == VK_RETURN ) ScriptEdit->SetFocus();
}
//---------------------------------------------------------------------------
