...
TSettingsManager::TSettingsManager() : FSettingsLoaded(false), FInitialized(false) {
    FSettingsFilePath = Ioutils::TPath::Combine(
        Ioutils::TPath::GetDirectoryName(ParamStr(0)),
		"settings.ini");
...
String TSettingsManager::DetectSvenCoopPath() {
    ...
    try {
		std::unique_ptr<Registry::TRegistry> registry(new Registry::TRegistry(KEY_READ));
        registry->RootKey = HKEY_CURRENT_USER;

		if (registry->OpenKeyReadOnly("Software\\Valve\\Steam")) {
			if (registry->ValueExists("SteamPath")) {
                String steamPath = registry->ReadString("SteamPath");
                registry->CloseKey();

                if (!steamPath.IsEmpty()) {
					steamPath = TrimRight(steamPath);
					svenPath = Ioutils::TPath::Combine( steamPath, "steamapps\\common\\Sven Co-op");

					if (Ioutils::TDirectory::Exists(svenPath)) {
						return StringReplace( svenPath, L"/", L"\\", TReplaceFlags() << rfReplaceAll );
                    }
                }
            }
        }
        ...
        String commonPaths[] = {
            "C:\\Program Files\\Steam\\steamapps\\common\\Sven Co-Op",
            "C:\\Program Files (x86)\\Steam\\steamapps\\common\\Sven Co-Op",
            "D:\\SteamLibrary\\steamapps\\common\\Sven Co-Op"
        };
		...
		OutputDebugString(("Registry error: " + e.Message).c_str());
	}
#else
	try {
		...
		if (!homeDir.IsEmpty()) {
			String commonPaths[] = {
				Ioutils::TPath::Combine(homeDir, ".steam/steam/steamapps/common/Sven Co-Op"),
				Ioutils::TPath::Combine(homeDir, ".local/share/Steam/steamapps/common/Sven Co-Op"),
				Ioutils::TPath::Combine(homeDir, "Steam/steamapps/common/Sven Co-Op")
			};
...
void TSettingsManager::DeriveRelatedPaths() {
	if (!FSvenCoopPath.IsEmpty()) {
		String derivedPluginsFile = Ioutils::TPath::Combine(FSvenCoopPath,"svencoop\\default_plugins.txt");
		if (FPluginsFilePath.IsEmpty() || !ValidatePath(FPluginsFilePath, false)) FPluginsFilePath = StringReplace( derivedPluginsFile,L"/", L"\\", TReplaceFlags() << rfReplaceAll );
		String derivedPluginsFolder = Ioutils::TPath::Combine( FSvenCoopPath, "svencoop\\scripts\\plugins");
		if (FPluginsFolderPath.IsEmpty() || !ValidatePath(FPluginsFolderPath, true)) FPluginsFolderPath = StringReplace( derivedPluginsFolder,L"/", L"\\", TReplaceFlags() << rfReplaceAll );
    }
}
...
bool TSettingsManager::LoadSettings() {
    ...
    try {
        std::unique_ptr<TMemIniFile> iniFile(new TMemIniFile(FSettingsFilePath));

        FPluginsFilePath = iniFile->ReadString("Paths", "PluginsFile", FPluginsFilePath);
        FPluginsFolderPath = iniFile->ReadString("Paths", "PluginsFolder", FPluginsFolderPath);
        FProfilesFolderPath = iniFile->ReadString("Paths", "ProfilesFolder", FProfilesFolderPath);

        FFormWidth = iniFile->ReadInteger("UI", "FormWidth", FFormWidth);
        FFormHeight = iniFile->ReadInteger("UI", "FormHeight", FFormHeight);
		FSeparatorSize = iniFile->ReadFloat("UI", "SeparatorSize", FSeparatorSize);
		FDefaultProfile = iniFile->ReadBool( "UI", "DefaultProfile", FDefaultProfile );
...
bool TSettingsManager::SaveSettings() {
    try {
        String settingsDir = Ioutils::TPath::GetDirectoryName(FSettingsFilePath);
